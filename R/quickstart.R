...
quickstart <- function (data, rotate = c(NA, "flipX", "flipY", "rotateC", "rotateCC"), pc_criterion = "broken_stick", plot_sample = TRUE, message = TRUE) {
  ...
  if (message == TRUE) {
  message(paste0("The loaded data set contains ", summary$data_n, " specimen, delineated with ", summary$data_lm, " ", summary$data_dim,"-D landmarks. The ", summary$pc_crit, " criterion was used to select ", summary$pc_nPCs, " principal components."))}
  ...
  class(to_return) <- "quickstart"
  return(to_return)

}

#' Print for quickstart
#'
