...
## 1. Vector scores: `calc(shape)VS`{#vs}
...
You could either symmetrize templates before submitting them to quickstart (using `facefuns::symmTemplates`) which has the added benefit that it's very little fuss to plot the PCs on which the scores are based; or you could do so by using the option `symm = TRUE` in `calcShapeVS`.
...
## 2. Discriminant scores: `calcDS`{#ds}

Another way to calculate sexual dimorphism is by employing a linear discriminant analyses to predict group membership.

`calcDS` first conducts a (forward) stepwise variable selection and then carries out a linear discriminant analysis. It takes two arguments:
