...
:-------- | :---------------------------------------------------------------------- 
`facefuns2d` | Conducts GPA and PCA, selects and visualizes PCs 
`read_lmdata` | Read landmark data from TPS, webmorph_list or Webmorph template files
`read_ply` | Read vertex data from binary PLY files
`select_pcs` | Select PCs based on *broken stick*, *Kaiser Guttman* or *total variance explained* criterion 
`make_2dpcs` | Creates 2-D coordinates for a reference face at +/- x units of standard deviations for a given (list of) principal component(s) 
`make_3dpcs` | Creates 3-D coordinates for a reference face at +/- x units of standard deviations for a given (list of) principal component(s)
...
`calc_shapevs` | Calculate group membership based on vector projection (from facefuns2d object, allows for symmetrizing of templates) | [calcSD](./calcSD.html)
