...
This tutorial covers basic steps in working with 3D meshes that have been delineated and resampled according to a "standard" mesh, i.e. each mesh's tesselations are homologous across the entire sample.

The 3D faces included in `facefuns` are 12 composite faces that show average white female and male faces from 6 different age groups (18-25, 26-35, 36-45, 46-55, 56-65, 66-75). Each composite comprises of 10 individual faces that were collected as part of the ERC-funded KINSHIP project (#647910). Corresponding texture files were created in Psychomorph by averaging individual texture maps.[^frl]

[^frl]: 3-D models were delineated and warped in [MorphAnalyser](https://cherry.dcs.aber.ac.uk/morphanalyser/version2.4/launch2.4.html) [@tiddeman2000]. The Face Research Lab uses a [DI3D system](https://www.di4d.com) to capture 3-D images; see [here](https://osf.io/gs5wm/) for more info.
...
Click and drag the face to move it. Note that the texture file needs to be a PNG to load successfully. If you get a `Instructions "usemtl" ignored` error, you can either ignore it or remove the offending line from the OBJ in a text editor.
...
You can use `geomorph::read.ply` to import ASCII PLYs, or `Morpho::file2mesh` to import both ASCII and binary PLYs. The example faces included in `facefuns` are binary PLYs, so we use the package `Morpho` [@morpho] to read them:
...
*NOTE TO SELF* - when using simplified meshes (I broke them down by 50% to keep package installation small), convert_points_to_mesh is no longer doing a very nice job - needs incredibly high alpha, there are still holes and the faces look squishy. FIX THAT!
