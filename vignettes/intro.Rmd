...
We will use `facefuns::read_shapedata` to read the template files.
...
`summary(pca_output)` will return a list of _all_ principal components. That is usually not what we want. Instead, we will use `facefuns::selectPCs` to decide which PCs we would like to retain for subsequent analyses.
...
The principal component scores for each face are stored in `pca_output$x`. We will save them for any subsequent analyses we might want to do.
...
What do the shape PCs for the London Set look like? You can use `geomorph`s `plotRefToTarget` function to visualize differences, e.g., between the average face and the sample min and max values on individual PCs.
...
`quickstart` is a convenience function for doing all of the above in one step.
...
`quickstart` returns a list with the following elements:

* `data`: Procrustes- aligned data    
...
By default, `quickstart` will create all selected PCs at plus and minus 3 standard deviations. These can again be plotted using `plotRefToTarget` or using `plot2DPCs`. Note that while `plotRefToTarget` is based on the sample range of a specific PC, `plot2DPCs` is based on units of standard deviation (see also `help(plot2DPCs)`).

```{r, fig.width=9, fig.height=15, fig.show='hold', fig.align='center'}
plot2DPCs(input = shapedata$pc_plot,
          ref = shapedata$average,
          which_pcs = 1:2)
```

To create plots of any other PCs than the selected ones (or to visualize PCs at a different level of manipulation, e.g., 5 SDs), just enter the output from the PCA directly into `plot2DPCs`

```{r, fig.width=9, fig.height=15, fig.show='hold', fig.align='center'}
pca_output <- gm.prcomp(shapedata$data)

plot2DPCs(input = pca_output,
          ref = shapedata$average,
          which_pcs = c(10, 15, 20),
          vis_sd = 5)
```
