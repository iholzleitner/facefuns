test_that("warnings", {
  ...
  expect_error(plot2DPCs(input = pca_out, ref = ref,  which_pcs = 20:40), too_many)
})
...
test_that("basic", {
  ...
  # defaults (1 plot PC1)
  plot2DPCs(input = pca_out, ref = ref)

  # 1 plot PC2
  plot2DPCs(input = pca_out, ref = ref, 2)

  # 2 plots PC1:PC3
  plot2DPCs(input = pca_out, ref = ref, c(3, 1))
})
